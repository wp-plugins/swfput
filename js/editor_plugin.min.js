...
s+=' '+t+'="'+atts[t]+'"';}
return s+']'+c+'[/'+sc+']'},atts_filter:function(atts){var defs=SWFPut_video_utility_obj_def.prototype.defprops
outp={};for(var t in atts){if(defs[t]!==undefined){outp[t]=atts[t];}}
return outp;},date_now:function(){return Date.now?Date.now():new Date().getTime();},unixtime:function(){return(this.date_now()/1000);},loadtime_serial:0,get_new_putswf_shortcode:function(){var d=new Date(),s='[putswf_video url="" iimage="" altvideo=""]',e='[/putswf_video]',c='Edit me please! '
+d.toString()+', '
+d.getTime()+'ms';return s+c+e;},_wp:wp||false,attachment_data_by_id:function(id,result_cb){var pid=id,res={status:0,response:null};if(this._wp){this._wp.ajax.send('get-attachment',{data:{id:pid}})
.done(function(response){res.status=response?true:null;res.response=response;if(result_cb&&typeof result_cb==='function'){result_cb(id,res);}})
.fail(function(response){res.status=false;res.response=response;if(result_cb&&typeof result_cb==='function'){result_cb(id,res);}});}},attachments:{},get_attachment_by_id:function(id,attach_put,result_cb){if(this.attachments.id===undefined){if(attach_put!==undefined&&attach_put){this.attachments.id=attach_put;return attach_put;}else{var obj=this.attachments,cb=result_cb||false;this.attachment_data_by_id(id,function(_id,_res){if(_res.status===true){obj[_id]=_res.response;}else{obj[_id]=false;}
if(typeof cb==='function'){cb(_id,_res,obj);}});return null;}}else{if(attach_put!==undefined&&attach_put){this.attachments.id=attach_put;}
return this.attachments.id;}
return false;}};var SWFPut_video_utility_obj=new SWFPut_video_utility_obj_def(wp||false);function SWFPut_repl_nl(str){return str.replace(/\r\n/g,'\n').replace(/\r/g,'\n').replace(/\n/g,'<br />');};if(SWFPut_video_utility_obj._bbone_mvc_opt===true&&wp!==undefined&&wp.mce!==undefined&&wp.mce.views!==undefined&&wp.mce.views.register!==undefined&&typeof wp.mce.views.register==='function'){var SWFPut_add_button_func=function(btn){var ivl,ivlmax=100,tid=btn.id,sc=SWFPut_video_utility_obj.get_new_putswf_shortcode(),dat=SWFPut_putswf_video_inst.get_mce_dat(),enc=window.encodeURIComponent(sc),ed=dat.ed
div=false;if(!ed){alert('Failed to get visual editor');return false;}
ed.selection.setContent(sc+'&nbsp;',{format:'text'});ivl=setInterval(function(){var divel,got=false,$=ed.$;if(div===false){var w=$('.wpview-wrap');w.each(function(cur){var at;cur=w[cur];at=cur.getAttribute('data-wpview-text');if(at&&at.indexOf(enc)>=0){divel=cur;div=$(cur);return false;}});}
if(div!==false){var f=div.find('iframe');if(f){ed.selection.select(divel,true);ed.selection.scrollIntoView(divel);div.trigger('click');got=true;}}
if((--ivlmax<=0)||got){clearInterval(ivl);}},1500);return false;};var SWFPut_get_attachment_by_id=function(id,attach_put,result_cb){return SWFPut_video_utility_obj?SWFPut_video_utility_obj.get_attachment_by_id(id,attach_put,result_cb||false):false;};var SWFPut_cache_shortcode_ids=function(sc,cb){var aatt=[sc.get('url'),sc.get('altvideo'),sc.get('iimage')],_cb=(cb&&typeof cb==='function')?cb:false;_.each(aatt,function(s){if(s!=undefined){_.each(s.split('|'),function(t){var m=t.match(/^[ \t]*([0-9]+)[ \t]*$/);if(m&&m[1]){var res=SWFPut_get_attachment_by_id(m[1],false,_cb);if(res!==null&&_cb!==false){var o=SWFPut_video_utility_obj.attachments;cb(m[1],o[m[1]],o);}}});}});};var SWFPut_get_iframe_document=function(head,styles,bodcls,body){head=head||'';styles=styles||'';bodcls=bodcls||'';body=body||'';return('<!DOCTYPE html>'+
'<html>'+
'<head>'+
...
$(iframe.contentWindow).on('load',resize);if(MutationObserver){var n=iframeDoc;observer=new MutationObserver(_.debounce(resize,100));observer.observe(n,{attributes:true,childList:true,subtree:true});$(node).one('wp-mce-view-unbind',function(){observer.disconnect();});}else{for(i=1;i<6;i++){setTimeout(resize,i*700);}}
function classChange(){iframeDoc.body.className=editor.getBody().className;}
...
return ostr||str;},replaceMarkers:function(){this.getMarkers(function(editor,node){var c1=M.marker_comp_prepare($(node).html()),c2=M.marker_comp_prepare(this.text);if(c1!==c2){editor.dom.setAttrib(node,'data-wpview-marker',null);return;}
editor.dom.replace(editor.dom.createFragment('<div class="wpview-wrap" data-wpview-text="'+this.encodedText+'" data-wpview-type="'+this.type+'">'+
'<p class="wpview-selection-before">\u00a0</p>'+
'<div class="wpview-body" contenteditable="false">'+
'<div class="wpview-content wpview-type-'+this.type+'"></div>'+
'</div>'+
'<p class="wpview-selection-after">\u00a0</p>'+
'</div>'),node);});},match:function(content){var rx=/\[(\[?)(putswf_video)(?![\w-])([^\]\/]*(?:\/(?!\])[^\]\/]*)*?)(?:(\/)\]|\](?:([^\[]*(?:\[(?!\/\2\])[^\[]*)*)(\[\/\2\]))?)(\]?)/g,match=rx.exec(content);if(match){var c1,c2;c1=' capencoded="'+encodeURIComponent(match[5])+'"';c2=match[3].indexOf(' capencoded=');if(c2<0){c2=match[3]+c1;}else{c2=match[3].replace(/ capencoded="[^"]*"/g,c1);}
...
update(s);});});frame.on('close',function(){frame.detach();});frame.open();}};var V=_.extend({},M,{action:'parse_putswf_video_shortcode',initialize:function(){var self=this;this.fetch();this.getEditors(function(editor){editor.on('wpview-selected',function(){self.pausePlayers();});});},fetch:function(){var self=this,atts=SWFPut_video_utility_obj.atts_filter(this.shortcode.attrs.named),sc=SWFPut_video_utility_obj.mk_shortcode(this.shortcode.tag,atts,this.shortcode.content),ng=this.shortcode.string();wp.ajax.send(this.action,{data:{post_ID:$('#post_ID').val()||0,type:this.shortcode.tag,shortcode:sc}})
.done(function(response){self.render(response);})
.fail(function(response){if(self.url){self.removeMarkers();}else{self.setError(response.message||response.statusText,'admin-media');}});},stopPlayers:function(event_arg){var rem=event_arg;this.getNodes(function(editor,node,content){var p,win,iframe=$('iframe.wpview-sandbox',content).get(0);if(iframe&&(win=iframe.contentWindow)&&win.evhh5v_sizer_instances){try{for(p in win.evhh5v_sizer_instances){var vi=win.evhh5v_sizer_instances[p],v=vi.va_o||false,f=vi.o||false,act=(event_arg==='pause')?'pause':'stop';if(v&&(typeof v[act]==='function')){v[act]();}
if(f&&(typeof f[act]==='function')){f[act]();}}}catch(err){var e=err.message;}
if(rem==='remove'){iframe.contentWindow=null;iframe=null;}}});},pausePlayers:function(){this.stopPlayers&&this.stopPlayers('pause');},});mce.views.register('putswf_video',_.extend({},V,{state:['putswf_video-details']}));}else{var view_def=mce.putswfMixin={View:_.extend({},av,{overlay:true,action:'parse_putswf_video_shortcode',initialize:function(options){var self=this;if(options&&options.shortcode){console.log('VIEW OPTIONS SHORTCODE: '+options.shortcode);this.shortcode=options.shortcode;}
this.cache_shortcode_ids(this.shortcode);_.bindAll(this,'setIframes','setNodes','fetch','stopPlayers');$(this).on('ready',this.setNodes);$(document).on('media:edit',this.stopPlayers);this.fetch();this.getEditors(function(editor){editor.on('hide',self.stopPlayers);});},cache_shortcode_ids:SWFPut_cache_shortcode_ids,setIframes:function(head,body){var MutationObserver=window.MutationObserver||window.WebKitMutationObserver||window.MozMutationObserver||false,importStyles=true,ivlmax=180,ivl;if(head||body.indexOf('<script')!==-1){this.getNodes(function(editor,node,content){var dom=editor.dom,styles='',bodyClasses=editor.getBody().className||'',iframe,iframeDoc,i,resize;content.innerHTML='';head=head||'';if(importStyles){if(!wp.mce.views.sandboxStyles){tinymce.each(dom.$('link[rel="stylesheet"]',editor.getDoc().head),function(link){if(link.href&&link.href.indexOf('skins/lightgray/content.min.css')===-1&&link.href.indexOf('skins/wordpress/wp-content.css')===-1){styles+=dom.getOuterHTML(link)+'\n';}});wp.mce.views.sandboxStyles=styles;}else{styles=wp.mce.views.sandboxStyles;}}
setTimeout(function(){ivl=setInterval(function(){if(--ivlmax<=0){clearInterval(ivl);return;}
iframe=dom.add(content,'iframe',{src:tinymce.Env.ie?'javascript:""':'',frameBorder:'0',allowTransparency:'true',scrolling:'no','class':'wpview-sandbox',style:{width:'100%',display:'block'}})||false;if(!iframe){return;}
clearInterval(ivl);ivlmax*=4;ivl=setInterval(function(){if(--ivlmax<=0){clearInterval(ivl);return;}
iframeDoc=iframe.contentWindow?(iframe.contentWindow.document||false):false;if(!iframeDoc){return;}
iframeDoc.open();iframeDoc.write(SWFPut_get_iframe_document(head,styles,bodyClasses,body));iframeDoc.close();resize=function(){var h;if(!iframe.contentWindow){return;}
h=$(iframeDoc.body).height();$(iframe).height(h);};try{if(MutationObserver){var nod=iframeDoc;new MutationObserver(_.debounce(function(){resize();},100))
.observe(nod,{attributes:true,childList:true,subtree:true});}else{throw ReferenceError('MutationObserver not supported');}}catch(exptn){if(exptn.message.length>0){console.log('Exception: '+exptn.message);}
for(i=1;i<36;i++){setTimeout(resize,1000);}}
if(importStyles){editor.on('wp-body-class-change',function(){iframeDoc.body.className=editor.getBody().className;});}
clearInterval(ivl);},250);},1000);},100);});}else{this.setContent(body);}},setNodes:function(){if(this.parsed){this.setIframes(this.parsed.head,this.parsed.body);}else{this.fail();}},fetch:function(){var self=this,atts=SWFPut_video_utility_obj.atts_filter(this.shortcode.attrs.named),sc=SWFPut_video_utility_obj.mk_shortcode(this.shortcode.tag,atts,this.shortcode.content),ng=this.shortcode.string();wp.ajax.send(this.action,{data:{post_ID:$('#post_ID').val()||0,type:this.shortcode.tag,shortcode:sc}})
.done(function(response){if(response){self.parsed=response;self.setIframes(response.head,response.body);}else{self.fail(true);}})
.fail(function(response){self.fail(response||true);});}
,fail:function(error){if(!this.error){if(error){this.error=error;}else{return;}}
if(this.error.message){if((this.error.type==='not-embeddable'&&this.type==='embed')||this.error.type==='not-ssl'||this.error.type==='no-items'){this.setError(this.error.message,'admin-media');}else{this.setContent('<p>'+this.original+'</p>','replace');}}else if(this.error.statusText){this.setError(this.error.statusText,'admin-media');}else if(this.original){this.setContent('<p>'+this.original+'</p>','replace');}},stopPlayers:function(event_arg){var rem=event_arg;this.getNodes(function(editor,node,content){var p,win,iframe=$('iframe.wpview-sandbox',content).get(0);if(iframe&&(win=iframe.contentWindow)&&win.evhh5v_sizer_instances){try{for(p in win.evhh5v_sizer_instances){var vi=win.evhh5v_sizer_instances[p],v=vi.va_o||false,f=vi.o||false,act=(event_arg==='pause')?'pause':'stop';if(v&&(typeof v[act]==='function')){v[act]();}
if(f&&(typeof f[act]==='function')){f[act]();}}}catch(err){var e=err.message;}
...
dat.qs=qs;dat.caption=cap||'';return dat;};var _sc_atts2if=function(url,dat,id,cap){var qs=dat.qs;var w=parseInt(dat.width),h=parseInt(dat.height);var dlw=w+60,fw=w+16,fh=h+16;var sty='width: '+dlw+'px';var att='width="'+fw+'" height="'+fh+'" '+
'sandbox="allow-same-origin allow-pointer-lock allow-scripts" '+
'';cap=dat.caption;if(cap==''){cap=fpo.ent;}
var cls=' align'+dat.align;var cldl='wp-caption evh-pseudo-dl '+cls;var cldt='wp-caption-dt evh-pseudo-dt';var cldd='wp-caption-dd evh-pseudo-dd';var r='';r+='<dl id="dl-'+id+'" class="'+cldl+'" style="'+sty+'">';r+='<dt id="dt-'+id+'" class="'+cldt+'" data-no-stripme="sigh">';r+='<evhfrm id="'+id+'" class="evh-pseudo" '+att+' src="';r+=url+'?'+qs;r+='"></evhfrm>';r+='</dt><dd id="dd-'+id+'" class="'+cldd+'">';r+=cap+'</dd></dl>';dat.code=r;return dat;};var parseShortcode=function(content){var uri=urlfm;return content.replace(/([\r\n]*)?(<p>)?(\[putswf_video([^\]]+)\]([\s\S]*?)\[\/putswf_video\])(<\/p>)?([\r\n]*)?/g,function(a,n1,p1,b,c,e,p2,n2){var sc=b,atts=c,cap=e;var ky=newkey();sc_map[ky]={};sc_map[ky].sc=sc;sc_map[ky].p1=p1||'';sc_map[ky].p2=p2||'';sc_map[ky].n1=n1||'';sc_map[ky].n2=n2||'';var dat=_sc_atts2qs(atts,cap);dat=_sc_atts2if(uri,dat,'evh-'+ky,cap);var w=dat.width,h=dat.height;var dlw=parseInt(w)+60;var cls='evhTemp mceIE'+dat.align
+' align'+dat.align;var r=n1||'';r+=p1||'';r+='<div id="evh-sc-'+ky+'" class="'+cls+'" style="width: '+dlw+'px">';r+=dat.code;r+='</div>';r+=p2||'';r+=n2||'';return r;});};var getShortcode=function(content){return content.replace(/<div ([^>]*class="evhTemp[^>]*)>((.*?)<\/div>)/g,function(a,att,lazy,cnt){var ky=att.match(/id="evh-sc-([0-9]+)"/);if(ky&&ky[1]){ky=ky[1];}else{return a;}
var sc='',p1='',p2='',n1='',n2='';if(sc_map[ky]){sc=sc_map[ky].sc||'';p1=sc_map[ky].p1||'';p2=sc_map[ky].p2||'';n1=sc_map[ky].n1||'';n2=sc_map[ky].n2||'';if(cnt){cnt=cnt.replace(/([\r\n]|<br[^>]*>)*/,'');var m=/.*<dd[^>]*>(.*)<\/dd>.*/.exec(cnt);if(m&&(m=m[1])){if(fpo.is(m,0)){m='';}
sc=sc.replace(/^(.*\]).*(\[\/[a-zA-Z0-9_-]+\])$/,function(a,scbase,scclose){return scbase+m+scclose;});sc_map[ky].sc=sc;}}}
if(!sc||sc===''){return a;}
return n1+p1+sc+p2+n2;});};return{_do_shcode:parseShortcode,_get_shcode:getShortcode};});}