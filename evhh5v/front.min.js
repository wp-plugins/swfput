function evhh5v_controlbar_elements(parms,fixups){var vidobj=evhh5v_controlbar_elements_check(parms,false);if(!vidobj){return;}
var ip=parms["iparm"];var num=ip["uniq"];var ivid=ip["vidid"];var op=parms["oparm"];if(op["std"]["preload"]!=="none"){vidobj.setAttribute("preload","none");}
vidobj.removeAttribute("controls");var pdefs={"parentdiv":ip["parentdiv"],"auxdiv":ip["auxdiv"],"id":ip["id"]?ip["id"]:"evhh5v_ctlbar_svg_"+num,"ctlbardiv":ip["bardivid"]?ip["bardivid"]:"evhh5v_ctlbar_div_"+num,"parent":ip["barobjid"]?ip["barobjid"]:"evhh5v_ctlbar_obj_"+num,"role":ip["role"]?ip["role"]:"bar"};if(!op["uniq"]){op["uniq"]={};}
...
barobj.style.width=""+ip["width"]+"px";barobj.style.height=""+ip["barheight"]+"px";barobj.setAttribute("onload","evhh5v_ctlbarload(this, '"+pdiv+"'); return false;");barobj.setAttribute('type',"image/svg+xml");barobj.setAttribute("data",url+(evhh5v_need_svg_query()?"":q));p=document.createElement('p');p.innerHTML=ip["altmsg"];barobj.appendChild(p);bardiv.appendChild(barobj);var alldiv=document.getElementById(adiv);url=ip["buturl"];var butdiv;butdiv=document.createElement('div');butdiv.setAttribute('id',"b_"+op["uniq"]["ctlbardiv"]);butdiv.setAttribute('class',ip["divclass"]);barobj=document.createElement('object');barobj.setAttribute('id',"b_"+op["uniq"]["parent"]);barobj.setAttribute('class',ip["divclass"]);q="?"+"parentdiv="+pdiv;p=document.createElement('param');p.setAttribute('name',"parentdiv");p.setAttribute('value',pdiv);barobj.appendChild(p);q+="&"+"parent="+"b_"+op["uniq"]["parent"];p=document.createElement('param');p.setAttribute('name',"parent");p.setAttribute('value',"b_"+op["uniq"]["parent"]);barobj.appendChild(p);q+="&"+"ctlbardiv="+"b_"+op["uniq"]["ctlbardiv"];p=document.createElement('param');p.setAttribute('name',"ctlbardiv");p.setAttribute('value',"b_"+op["uniq"]["ctlbardiv"]);barobj.appendChild(p);q+="&"+"role=1st";p=document.createElement('param');p.setAttribute('name',"role");p.setAttribute('value',"1st");barobj.appendChild(p);barobj.setAttribute("onload","evhh5v_ctlbutload(this, '"+pdiv+"'); return false;");barobj.setAttribute('type',"image/svg+xml");barobj.setAttribute("data",url+(evhh5v_need_svg_query()?"":q));butdiv.appendChild(barobj);url=ip["volurl"];var voldiv;voldiv=document.createElement('div');voldiv.setAttribute('id',"v_"+op["uniq"]["ctlbardiv"]);voldiv.setAttribute('class',ip["divclass"]);barobj=document.createElement('object');barobj.setAttribute('id',"v_"+op["uniq"]["parent"]);barobj.setAttribute('class',ip["divclass"]);q="?"+"parentdiv="+pdiv;p=document.createElement('param');p.setAttribute('name',"parentdiv");p.setAttribute('value',pdiv);barobj.appendChild(p);q+="&"+"parent="+"v_"+op["uniq"]["parent"];p=document.createElement('param');p.setAttribute('name',"parent");p.setAttribute('value',"v_"+op["uniq"]["parent"]);barobj.appendChild(p);q+="&"+"ctlbardiv="+"v_"+op["uniq"]["ctlbardiv"];p=document.createElement('param');p.setAttribute('name',"ctlbardiv");p.setAttribute('value',"v_"+op["uniq"]["ctlbardiv"]);barobj.appendChild(p);q+="&"+"role=vol";p=document.createElement('param');p.setAttribute('name',"role");p.setAttribute('value',"vol");barobj.appendChild(p);barobj.setAttribute("onload","evhh5v_ctlvolload(this, '"+pdiv+"'); return false;");barobj.setAttribute('type',"image/svg+xml");barobj.setAttribute("data",url+(evhh5v_need_svg_query()?"":q));voldiv.appendChild(barobj);alldiv.appendChild(voldiv);alldiv.appendChild(bardiv);alldiv.appendChild(butdiv);if(fixups!==undefined&&fixups==true){evhh5v_fixup_elements(parms);}};function evhh5v_controlbar_elements_check(parms,vidobj){if(!vidobj){vidobj=document.getElementById(parms["iparm"]["vidid"]);}
if(!vidobj){return false;}
var swfobj=false;var ss=[];ss.push(vidobj);for(var i=0;i<vidobj.childNodes.length;i++){var t=vidobj.childNodes.item(i);var nnlc=t.nodeName.toLowerCase();if(nnlc=='object'){if(parms.flashid!==undefined&&parms.flashid===t.id){swfobj=evhh5v_get_flashsupport()?t:false;}
continue;}
if(nnlc=='source'){ss.push(t);}}
var sfs=[];var maybe=0,probably=0,notype=0,nsource=0;while(ss.length){var add=false,sff=false;var o=ss.shift();var s=o.getAttribute('src');var t=o.getAttribute('type');if(!s||s.length<1){continue;}
nsource++;if(!t||t.length<1){if(s.match(/.*\.(mp4|m4v|mv4)[ \t]*$/i)){t='video/mp4';add=true;sff=s;sfs.push(sff);}else if(s.match(/.*\.(og[gv]|vorbis)[ \t]*$/i)){t='video/ogg';add=true;}else if(s.match(/.*\.(webm|wbm|vp[89])[ \t]*$/i)){t='video/webm';add=true;}else if(s.match(/.*\.(flv)[ \t]*$/i)){sff=s;sfs.push(sff);}}
if(!t||t.length<1){notype++;continue;}
if(!sff&&t.match(/.*video\/(mp4|flv).*/i)){sff=s;sfs.push(sff);}
var can=vidobj.canPlayType(t);if(can=='probably'){probably++;}else if(can=='maybe'){maybe++;}else{add=false;}
if(add){o.setAttribute('type',t);}}
if(probably>0||maybe>0){return vidobj;}
if(swfobj!==false){var aux=vidobj.parentNode;var par=aux.parentNode;vidobj.removeChild(swfobj);var ch=[];for(var i=0;i<swfobj.childNodes.length;i++){ch.push(swfobj.childNodes.item(i));}
while(ch.length){var t=ch.shift();var nnlc=t.nodeName.toLowerCase();if(nnlc=='param'){continue;}
swfobj.removeChild(t);vidobj.appendChild(t);}
par.replaceChild(swfobj,aux);swfobj.appendChild(aux);if(window.addEventListener)
window.addEventListener('load',function(e){var id=swfobj.id;try{if(swfobj.get_ack(id)!=id){evhh5v_msg('FAILED evhswf ack from "'+id+'"');return;}
for(var i=0,mx=sfs.length;i<mx;i++){var t=encodeURI(sfs[i]);swfobj.add_alt_url(t,true);}}catch(ex){evhh5v_msg('EXCEPTION calling evhswf: "'+ex.message+'"');}},false);return false;}
if(parms.flashid&&evhh5v_get_flashsupport()){swfobj=vidobj.parentNode.parentNode;if(swfobj.nodeName.toLowerCase()==='object'&&parms.flashid===swfobj.id){if(window.addEventListener)
window.addEventListener('load',function(e){var id=swfobj.id;try{if(swfobj.get_ack(id)!=id){evhh5v_msg('FAILED evhswf ack from "'+id+'"');return;}
for(var i=0,mx=sfs.length;i<mx;i++){var t=encodeURI(sfs[i]);swfobj.add_alt_url(t,true);}}catch(ex){evhh5v_msg('EXCEPTION calling evhswf: "'+ex.message+'"');}},false);return false;}}
...
evhh5v_sizer_instances[i].handle_resize();}};(function(){if(window.addEventListener){var sizer_event_time=250,tmo=false,f=function(e){tmo=tmo||setTimeout(function(){tmo=false;evhh5v_sizer_event_relay(e.type==="load");},sizer_event_time);};document.addEventListener("load",f,true);window.addEventListener("load",f,true);window.addEventListener("resize",f,true);}else{var onlddpre=document.onload;var onldwpre=window.onload;var onszwpre=window.onresize;document.onload=function(){if(typeof evhh5v_video_onlddpre==='function'){onlddpre();}
evhh5v_sizer_event_relay(true);};window.onload=function(){if(typeof evhh5v_video_onldwpre==='function'){onldwpre();}
evhh5v_sizer_event_relay(true);};window.onresize=function(){if(typeof evhh5v_video_onszwpre==='function'){onszwpre();}
evhh5v_sizer_event_relay(false);};}}());var evhh5v_sizer=function(dv,ob,av,ai,bld){this.ia_rat=1;this.hpad=0;this.vpad=0;this.wdiv=null;this.bld=null;this.inresize=0;this.d=document.getElementById(dv);if(!this.d){return;}
this.o=document.getElementById(ob);this.va_o=document.getElementById(av);this.ia_o=document.getElementById(ai);this.get_pads();this.wdiv=this.d.offsetWidth;if(this.ia_o&&this.ia_o.width>1){this.ia_rat=this.ia_o.width/this.ia_o.height;}
...
if(!bar){evhh5v_msg("BAD CTLBAR == "+bar);return;}
this.ctlbar=bar;this.va_o=new evhh5v_controller(this.va_o,bar,0);this.va_o.mk();},_style:function(el,sty){return evhh5v_getstyle(el,sty);},get_pads:function(el){var p=this._style(this.d,"padding-left")||0;this.hpad=parseInt(p);p=this._style(this.d,"paddin g-right")||0;this.hpad+=parseInt(p);p=this._style(this.d,"padding-top")||0;this.vpad=parseInt(p);p=this._style(this.d,"padding-bottom")||0;this.vpad+=parseInt(p);},handle_resize:function(){if(!this.d||this.inresize!=0)
return;var dv=this.d;var wo=this.wdiv;var wn=dv.offsetWidth;if(false&&wn==wo){return;}
this.wdiv=wn;this.get_pads();this.resize();},_int_rsz:function(o){var dv=this.d;if(!dv){return;}
var wd=this.wdiv;if(!wd){return;}
var np=0;var ho=o.height;var wo=o.width;var r=wo/ho;var vd=evhh5v_view_dims();var maxh=vd.height-16;try{if(evhh5v_sizer_maxheight_off!==undefined&&evhh5v_sizer_maxheight_off){maxh=wd/r+1;}}catch(ex){}
if((wd/r)>maxh){wd=Math.round(maxh*r);}
wd=Math.min(wd,vd.width);np=Math.round(Math.max((this.wdiv-wd)/2-0.5,0));wo=wd;ho=Math.round(wo/r);o.height=ho;o.width=wo;try{if(o.pixelHeight!==undefined){o.pixelHeight=ho;o.pixelWidth=wo;}}catch(ex){}
np=""+np+'px';dv.style.paddingLeft=np;dv.style.paddingRight=np;},_int_imgrsz:function(o){if(o.complete!==undefined&&!o.complete){return;}
...
var wd=this.wdiv;if(wd==null){return;}
wd-=this.hpad;var rd=this.ia_rat;var ri=o.naturalWidth/o.naturalHeight;if(rd>ri){o.height=Math.round(wd/rd);o.width=Math.round(o.height*ri);}else{o.width=wd;o.height=Math.round(wd/ri);}},resize:function(){if(!this.d){return;}
this.inresize=1;if(this.o){this._int_rsz(this.o);}
if(this.va_o){this._int_rsz(this.va_o);}
if(this.ia_o){this._int_imgrsz(this.ia_o);}
this.inresize=0;}};var evhh5v_fullscreen={if_defined:function(){return(this.get_symset_key()!==false);},capable:function(){try{return!(!this.enabled());}
...
]}};function evhh5v_fullscreen_ok(){return evhh5v_fullscreen.if_defined();}
...
return false;};var hdlmu=function(e){if(that.volctl_mousedown){var t=this;that.hdl_volctl(e,t);}
if(e.target.releaseCapture!==undefined){e.target.releaseCapture();}
...
return evhh5v_ua_is_mobile();},xfacts:[0.5,1.5,2,1.5,2],mk:function(){var mobi=this.is_mobile();var mnot=!mobi;var svg=this.svg;var doc=this.doc;var ofs=0;this.vol_horz=mobi;if(mobi){this.xfacts=[0.25,1.75,1.75,1.75,1.75];}
this.button_data={};var dat=this.button_data;svg.setAttribute("viewBox",this.viewbox);this.gall=doc.getElementById("g_all_g");var gbg=doc.getElementById("ctlbar_bg");this.bgrect=this.mk_bgrect(gbg);var gbn=doc.getElementById("g_button_1");dat["show"]=gbn;dat["hide"]=doc.getElementById("g_button_2");dat["hide"].setAttribute("visibility","hidden");dat["play"]={};dat["play"].defx=ofs+=this.xfacts[0];this.button_play=this.mk_playpause(gbn,dat["play"].defx);dat["play"].obj=this.button_play;dat["stop"]={};dat["stop"].defx=ofs+=this.xfacts[1];this.button_stop=this.mk_stop(gbn,dat["stop"].defx);dat["stop"].obj=this.button_stop;this.stopbtn_disab();if(true||mnot){dat["doscale"]={};dat["doscale"].defx=ofs+=this.xfacts[2];this.button_doscale=this.mk_doscale(gbn,dat["doscale"].defx);dat["doscale"].obj=this.button_doscale;this.show_scalein();this.blur_doscale();dat["fullscreen"]={};dat["fullscreen"].defx=ofs+=this.xfacts[3];this.button_fullscreen=this.mk_fullscreen(gbn,dat["fullscreen"].defx);dat["fullscreen"].obj=this.button_fullscreen;this.show_fullscreenout();this.blur_fullscreen();}else{this.button_doscale=this.button_fullscreen=false;}
dat["volume"]={};dat["volume"].defx=ofs+=this.xfacts[4];this.button_volume=this.mk_volume(gbn,dat["volume"].defx);dat["volume"].obj=this.button_volume;for(var k in dat){var cur=dat[k];if(cur.defx===undefined){continue;}
var obj=cur.obj;cur.defx=obj.getAttribute("x");obj.x.baseVal.convertToSpecifiedUnits(obj.x.baseVal.SVG_LENGTHTYPE_PX);cur.defx_px=obj.x.baseVal.valueInSpecifiedUnits;obj.width.baseVal.convertToSpecifiedUnits(obj.width.baseVal.SVG_LENGTHTYPE_PX);cur.defwidth_px=obj.width.baseVal.valueInSpecifiedUnits;}
var gpl=doc.getElementById("prog_seek");this.progress_play=this.mk_prog_pl(gpl);var gdl=doc.getElementById("prog_load");this.progress_load=this.mk_prog_dl(gdl);this.init_inibut();this.init_volctl();this.OK=true;},set_bar_visibility:function(visi){this.svg.setAttribute("visibility",visi);},set_narrow:function(narrow){var dat=this.button_data;if(dat["doscale"]==undefined||dat["fullscreen"]==undefined){return;}
if(dat["doscale"].hidden&&narrow){return;}
if(!dat["doscale"].hidden&&!narrow){return;}
var gs=dat["show"],gh=dat["hide"];if(narrow){var x=dat["doscale"].obj.getAttribute("x");gs.removeChild(dat["doscale"].obj);gs.removeChild(dat["fullscreen"].obj);dat["doscale"].hidden=true;dat["volume"].obj.setAttribute("x",x);return;}
dat["doscale"].hidden=false;dat["volume"].obj.setAttribute("x",dat["volume"].defx);gs.insertBefore(dat["fullscreen"].obj,dat["volume"].obj);gs.insertBefore(dat["doscale"].obj,dat["fullscreen"].obj);},show_waitanim:function(x,y){if(!this.init_inibut()){return false;}
...
this.doshowbar=true;if(this.params['disablebar']!==undefined&&this.params['disablebar']=='true'){this.disablebar=true;this.doshowbar=false;this.doshowbartime=false;}
this.allowfull=true;if(this.params['allowfull']!==undefined&&this.params['allowfull']=='false'){this.allowfull=false;}
...
if(this.allowfull&&evhh5v_fullscreen_ok()){this.bar.unblur_fullscreen();}else{this.bar.blur_fullscreen();}
...
this.setup_canvas();this.install_handlers();var that=this;this.bar.controller_handle_volume=function(pct){that.ptrtick=0;if(!isFinite(pct)){return;}
var v=that._vid;if(v.volume!==undefined){pct=Math.max(0,Math.min(1,pct));v.volume=that.init_vol=pct;}}
this.bar.scale_volctl(1);if(this.autoplay){this._vid.setAttribute("preload","metadata");}else{this._vid.setAttribute("preload",this.params["preload"]);var f=function(){if(that.has_been_played){that.bar.hide_inibut();return;}
...
var ename=evt.type;var map=that.handlermap;if(map[ename]!=undefined){for(var i=0,mx=map[ename].length;i<mx;i++){var f=map[ename][i];if(f&&typeof(f)=="function"){f.call(that,evt);}}}},_obj_add_evt:function(obj,bubool){if(typeof(bubool)!=="boolean"){bubool=false;}
for(var k in this.handlermap){obj.addEventListener(k,this.callbk,bubool);}},add_evt:function(ename,callbk,bubool){var inst=false;if(this.handlermap[ename]==undefined){this.handlermap[ename]=[];inst=true;}
this.handlermap[ename].push(callbk);if(inst&&this._vid){this._vid.addEventListener(ename,this.callbk,bubool);}},addEventListener:function(ename,callbk,bubool){if(typeof(bubool)!=="boolean"){bubool=false;}
if(typeof ename==="string"){this.add_evt(ename,callbk,bubool);}else if(ename instanceof Array){var mx=ename.length;for(var i=0;i<mx;i++){this.add_evt(ename[i],callbk,bubool);}}},install_handlers:function(newvid){var newv=false;if(newvid===true){newv=true;this.handlermap={};}
var wait_ev=["waiting"];if(/Chrom(e|ium)\/([0-2][0-9]|3[0-2])\./i.test(navigator["userAgent"])){wait_ev.push("seeking");}
this.addEventListener(wait_ev,function(e){this.show_wait();},false);this.addEventListener(["seeked","canplaythrough","playing","loadeddata","ended"],function(e){this.hide_wait();},false);this.addEventListener(["ended"],function(e){if(this.evcnt!==undefined){for(var k in this.evcnt){evhh5v_msg("EVENT count for '"+k+"': "+this.evcnt[k]);this.evcnt[k]=0;}}},false);this.addEventListener("play",function(e){this.get_canvas_context();this.canvas_clear();this.has_been_played=true;this.stop_forced=false;this.playing=true;this.bar.hide_inibut();this.put_canvas_frame();this.bar.show_pauseico();this.bar.stopbtn_enab();this.showhideBar(this.doshowbar=false);},false);this.addEventListener("pause",function(e){this.end_canvas_frame();this.playing=false;this.bar.show_playico();this.bar.stopbtn_enab();this.hide_wait();if(this.stop_invoked_proc){var f=this.stop_invoked_proc;this.stop_invoked_proc=false;f.call(this);}},false);this.addEventListener("playing",function(e){this.playing=true;this.bar.show_pauseico();this.bar.stopbtn_enab();},false);this.addEventListener("suspend",function(e){if(!this.susptimer){var that=this.bar;this.susptimer=setTimeout(function(){that.show_dl_inactive();},3000);}},false);this.addEventListener("progress",function(e){if(this.susptimer){clearTimeout(this.susptimer);this.susptimer=false;var that=this.bar;this.susptimer=setTimeout(function(){that.show_dl_inactive();},3000);}
this.bar.show_dl_active();},false);this.addEventListener(["loadedmetadata","loadeddata","emptied"],function(e){this.bar.show_dl_inactive();},false);this.addEventListener(["loadedmetadata","resize"],function(e){if(e.type==="loadedmetadata"){this.on_metadata();this.gotmetadata=true;}else if(e.type==="resize"){if(false){evhh5v_msg("Got RESIZE: w == "+
...
if(e.type==="error"&&!this._vid.error){return;}else if(e.type!=="ended"){var t=this._vid.error;if(!t){return;}
try{switch(t.code){case MediaError.MEDIA_ERR_NETWORK:alert("A network error stopped the media fetch; "
...
this.mouse_show();this.ptrtick=0;break;case"click":e.stopPropagation();this.playpause();break;case"dblclick":break;default:evhh5v_msg("GOT MOUSE EVENT: "+e.type);}},false);var kbevt=["keyup","keydown"];this.addEventListener(kbevt,function(e){e.stopPropagation();switch(e.type){case"keydown":this.curkey=e.keyCode;break;case"keyup":if(this.curkey==32){this.playpause();}else if(this.curkey==81||this.curkey==113){this.stop();}else if(this.curkey==70||this.curkey==102){if(this.allowfull){this.fullscreen();}}else if(this.curkey==71||this.curkey==103){if(this.dbg_key===undefined){this.dbg_key=false;}
...
if(!this.bar.volctl_mousedown){if(this.rekonq_mousebug)
this.rekonq_mousebug--;if(++this.ptrtick>=this.ptrtickmax){this.rekonq_mousebug=parseInt(this.ptrtickmax/10);this.mouse_hide();if(this.doshowbartime){this.showhideBar(this.doshowbar=false);}
this.ptrtick=0;}}else{this.ptrtick=0;}
var prgupd=this.ntick&1;if(prgupd&&!(this._vid.paused||this._vid.ended)){this.play_progress_update();}
...
tv.setAttribute("preload",poster?"none":(this.gotmetadata?"metadata":"none"));while(this._vid.hasChildNodes()){var tn=this._vid.firstChild.cloneNode(true);tv.appendChild(tn);this._vid.removeChild(this._vid.firstChild);}
if(!this.is_canvas){this._vid.parentNode.replaceChild(tv,this._vid);}
this._vid.src=null;this._vid.removeAttribute("src");try{this._vid.currentSrc=null;}catch(e){}
this._vid.load();try{delete this._vid;}catch(e){}
this._vid=tv;this._vid.evhh5v_controller=this;this.setup_aspect_factors();this._obj_add_evt(this._vid);this.gotmetadata=this.playing=false;if(poster){this._vid.setAttribute("poster",poster);}
this.put_canvas_poster();this.bar.show_playico();this.bar.progress_pl(1);this.bar.stopbtn_disab();};if(this._vid.paused||this._vid.ended){proc.call(this);}else{this.stop_invoked_proc=proc;this._vid.pause();}},do_scale:function(){this.doscale=!this.doscale;this.setup_aspect_factors();this.put_canvas_frame_single();if(this.doscale){this.bar.show_scalein();}else{this.bar.show_scaleout();}},fullscreen:function(){if(!(this.allowfull&&evhh5v_fullscreen.capable())){this.bar.blur_fullscreen();if(this.allowfull){alert("Full screen mode is not available.");}
return;}
...
if(!this.volctl_showing){this.show_volctl();}else{this.hide_volctl();}},show_volctl:function(bot){if(!this.volctl_showing){if(bot==undefined)
...
function evhh5v_need_svg_query(){if(document.evhh5v_need_svg_query_bool!==undefined){return document.evhh5v_need_svg_query_bool;}
...
return document.evhh5v_ua_is_mobile_bool;};function evhh5v_fixup_elements(parms){var ip=parms["iparm"];if(/Opera/i.test(navigator["userAgent"])){var t=document.getElementById(ip["auxdiv"]);if(t&&t.parentNode.nodeName.toLowerCase()==="object"){var p=t.parentNode;var d=p.parentNode;p.removeChild(t);d.replaceChild(t,p);}}}
...
return v;};var evhh5v_get_flashsupport=function(el,sty){if(document.evhh5v_get_flashsupport_found===undefined){if(!navigator.plugins["Shockwave Flash"]){document.evhh5v_get_flashsupport_found=false;}else{document.evhh5v_get_flashsupport_found=true;}}
return document.evhh5v_get_flashsupport_found;}
var evhh5v_msg_off=true;var evhh5v_msg=function(msg,cons,pfx){if(evhh5v_msg_off){return;}
var m=(pfx||"EVHMSG: ")+msg;if((cons!==undefined&&cons)||typeof window.dump!=='function'){console.log(m);}else{window.dump(m+"\n");}};var evhh5v_view_horrible_dim_hack_result=null;var evhh5v_view_horrible_dim_hack=function(){if(evhh5v_view_horrible_dim_hack_result===null){var d=document.documentElement;if(d&&d.clientHeight===0){evhh5v_view_horrible_dim_hack_result=true;}}
if(evhh5v_view_horrible_dim_hack_result===null){var d=document,div=d.createElement('div');div.style.height="9000px";d.body.insertBefore(div,d.body.firstChild);evhh5v_view_horrible_dim_hack_result=(d.documentElement.clientHeight>8800);d.body.removeChild(div);}
return evhh5v_view_horrible_dim_hack_result;}
var evhh5v_view_dims=function(){var d={};if(typeof document.clientHeight==='number'){d["width"]=document.clientWidth;d["height"]=document.clientHeight;}else if(evhh5v_view_horrible_dim_hack()){d["width"]=document.body.clientWidth;d["height"]=document.body.clientHeight;}else{d["width"]=document.documentElement.clientWidth;d["height"]=document.documentElement.clientHeight;}
return d;};