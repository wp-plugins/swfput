...
window.addEventListener('load',function(e){var id=swfobj.id;try{if(swfobj.get_ack(id)!=id){console.log('FAILED evhswf ack from "'+id+'"');return;}
for(var i=0,mx=sfs.length;i<mx;i++){var t=encodeURI(sfs[i]);swfobj.add_alt_url(t,true);}}catch(ex){console.log('EXCEPTION calling evhswf: "'+ex.message+'"');}},false);return false;}
...
var evhh5v_sizer=function(dv,ob,av,ai,bld){this.ia_rat=1;this.pad=0;this.wdiv=null;this.bld=null;this.inresize=0;this.d=document.getElementById(dv);if(!this.d){return;}
this.o=document.getElementById(ob);this.va_o=document.getElementById(av);this.ia_o=document.getElementById(ai);var p=this._style(this.d,"padding-left");if(p)
this.pad=Math.max(this.pad,parseInt(p));this.wdiv=this.d.offsetWidth;if(this.ia_o&&this.ia_o.width>1){this.ia_rat=this.ia_o.width/this.ia_o.height;}
...
this.ctlbar=bar;this.va_o=new evhh5v_controller(this.va_o,bar,0);this.va_o.mk();},_style:function(el,sty){return evhh5v_getstyle(el,sty);},handle_resize:function(){if(!this.d||this.inresize!=0)
...
this.resize();},_int_rsz:function(o){var wd=this.wdiv;if(!wd)
return;wd-=this.pad*2;var wo=o.width;if((wd-wo)==0)
return;var r=wo/o.height;wo=Math.round(wd/r);o.height=wo;o.width=wd;if(o.pixelHeight!==undefined){o.pixelHeight=wo;o.pixelWidth=wd;}},_int_imgrsz:function(o){if(o.complete!==undefined&&!o.complete){return;}
...
t.in_fullscreen=false;t.height=t.fs_dimstore[0];t.width=t.fs_dimstore[1];evhh5v_fullscreen.handle_change(t.orig_fs_change_func);evhh5v_fullscreen.handle_error(t.orig_fs_error_func);t.orig_fs_change_func=null;t.orig_fs_error_func=null;t.bar.show_fullscreenout();});this.orig_fs_error_func=evhh5v_fullscreen.handle_error(function(evt){evhh5v_fullscreen.handle_change(t.orig_fs_change_func);evhh5v_fullscreen.handle_error(t.orig_fs_error_func);t.orig_fs_change_func=null;t.orig_fs_error_func=null;alert("Full screen mode failed.");});evhh5v_fullscreen.request(p);}else if(el==p){evhh5v_fullscreen.exit();}}catch(ex){alert(ex.name+': "'+ex.message+'"');}},togglevolctl:function(){this.ptrtick=0;if(!this.volctl_showing){this.show_volctl();}else{this.hide_volctl();}},show_volctl:function(bot){if(!this.volctl_showing){if(bot==undefined)
